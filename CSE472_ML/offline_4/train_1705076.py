...
def take_input():
    """
    Take input from user and return it as a tuple of (epoch_cnt, learning_rates, validation_ratio, save_model)
    """
    epoch_cnt = int(input("Enter epoch count: "))
    learning_rates = [float(x) for x in input("Enter learning rates: ").split()]
    validation_ratio = float(input("Enter validation ratio: "))
    save_model = input("Save model? (y/n): ") == 'y'
    print()

    return epoch_cnt, learning_rates, validation_ratio, save_model
...
def split_data(X, y, validation_ratio):
    ...
    train_size = int((1 - validation_ratio) * X.shape[0])

    X_train = X[idx][:train_size]
    y_train = y[idx][:train_size]

    X_validation = X[idx][train_size:]
    y_validation = y[idx][train_size:]

    return X_train, y_train, X_validation, y_validation


def load_train_data(path_to_folder):
    """
    Load training data (a, b, c) from folder assuming csv has digits
    """
    ...
    for training_set in training_sets:
        image_folder = os.path.join(path_to_folder, training_set)
        csv_path = os.path.join(path_to_folder, training_set + '.csv')
        _, X_, y_ = load_image_data(image_folder, csv_path, image_shape=(28, 28), dilation_shape=(2, 2))
        X.append(X_)
        y.append(y_)
...
def save_scores(train_scores, validation_scores, filename):
    """
    Save scores to csv file
    """
    with open(filename, 'w', newline='') as f:
        writer = csv.writer(f)
        writer.writerow(['epoch', 'train_loss', 'train_accuracy', 'train_macro_f1', 'validation_loss', 'validation_accuracy', 'validation_macro_f1'])
        for i in range(len(train_scores)):
            writer.writerow([i] + train_scores[i] + validation_scores[i])
    return


def lenet(epochs, learning_rate, batch_size):
    """
    Create a LeNet-5 like CNN
    """
    cnn = CNN(epochs = epochs, learning_rate = learning_rate, batch_size = batch_size)

    cnn.addLayer(ConvolutionLayer(num_channels=6, filter_size=(5, 5), stride=(1, 1), pad=2))
    cnn.addLayer(ReLUActivationLayer())

    cnn.addLayer(MaxPoolingLayer(filter_size=(2, 2), stride=(2, 2)))

    cnn.addLayer(ConvolutionLayer(num_channels=16, filter_size=(5, 5), stride=(1, 1), pad=0))
    cnn.addLayer(ReLUActivationLayer())

    cnn.addLayer(MaxPoolingLayer(filter_size=(2, 2), stride=(2, 2)))
    cnn.addLayer(FlatteningLayer())
    
    cnn.addLayer(DenseLayer(out_node_count=120))
    cnn.addLayer(ReLUActivationLayer())

    cnn.addLayer(DenseLayer(out_node_count=84))
    cnn.addLayer(ReLUActivationLayer())

    cnn.addLayer(DenseLayer(out_node_count=10))
    cnn.addLayer(SoftMaxLayer())

    return cnn

if __name__ == '__main__':
    assert len(sys.argv) == 2, "Usage: python3 1705076_train.py <path_to_folder>"

    np.random.seed(42)

    epoch_cnt, learning_rates, validation_ratio, save_model = take_input()

    path_to_folder = sys.argv[1]
    X, y = load_train_data(path_to_folder)

    X_train, y_train, X_validation, y_validation = split_data(X, y, validation_ratio)
    ...
    for rate in learning_rates:
        print()
        print("    Training for learning rate = " + str(rate))
        print()

        cnn = lenet(epochs = epoch_cnt, learning_rate = rate, batch_size = 64)

        training_time_start = datetime.datetime.now()
        if validation_ratio > 0:
            train_scores, validation_scores = cnn.train(X_train, y_train, X_validation, y_validation, record = True, verbose = True)
        else:
            cnn.train(X_train, y_train, record = False, verbose = False)
        training_time_end = datetime.datetime.now()

        training_time = training_time_end - training_time_start
        print("Training took " + str(training_time))

        if validation_ratio > 0:
            file_name = 'scores.csv' if len(learning_rates) == 1 else 'scores_' + str(rate) + '.csv'
            save_scores(train_scores, validation_scores, file_name)

        if save_model:
            model_name = "1705076_model.pickle" if len(learning_rates) == 1 else "1705076_" + str(rate) + ".pickle"
