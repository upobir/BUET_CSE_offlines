...
class AES:
    ...
    def key_schedule(self, key : bytearray) -> List[BitVector]:
        ...
        for i in range((self.NUM_ROUNDS+1) * self.NUM_WORDS):
            word : BitVector
            if i < self.NUM_WORDS:
                word = BitVector(hexstring = key[4*i:4*i+4].hex())
                words.append(word)
        ...
        for i in range(0, len(words), self.NUM_WORDS):
            ...
            print(round_keys[-1].get_bitvector_in_hex())

        return round_keys

    def SubWord(self, word : BitVector) -> BitVector:
        s0 : BitVector = BitVector(intVal=self.SBOX[word[0 : 8].int_val()], size=8)
        s1 : BitVector = BitVector(intVal=self.SBOX[word[8 : 16].int_val()], size=8)
        s2 : BitVector = BitVector(intVal=self.SBOX[word[16 : 24].int_val()], size=8)
        s3 : BitVector = BitVector(intVal=self.SBOX[word[24 : 32].int_val()], size=8)
        return s0 + s1 + s2 + s3
