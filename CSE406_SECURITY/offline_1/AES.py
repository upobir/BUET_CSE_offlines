from pydoc import plain
from BitVector import *
from typing import List, Tuple

# check other AES

class AES:
    KEY_LENGTH : int = 16
    NUM_ROUNDS : int = 10
    NUM_WORDS : int = 4
    BLOCK_SIZE : int = 128
    BLOCK_SIZE_IN_BYTES : int = 16
    AES_MODULO : BitVector = BitVector(bitstring='100011011')
    PADDING = bytearray(' ', 'utf-8')
    SBOX : List[int] = [
        ,
    ]
    INV_SBOX : List[int] = [
        ,
    ]
    MIXER = [
        [BitVector(hexstring="02"), BitVector(hexstring="03"), BitVector(hexstring="01"), BitVector(hexstring="01")],
        [BitVector(hexstring="01"), BitVector(hexstring="02"), BitVector(hexstring="03"), BitVector(hexstring="01")],
        [BitVector(hexstring="01"), BitVector(hexstring="01"), BitVector(hexstring="02"), BitVector(hexstring="03")],
        [BitVector(hexstring="03"), BitVector(hexstring="01"), BitVector(hexstring="01"), BitVector(hexstring="02")]
    ]
    INV_MIXER = [
        [BitVector(hexstring="0E"), BitVector(hexstring="0B"), BitVector(hexstring="0D"), BitVector(hexstring="09")],
        [BitVector(hexstring="09"), BitVector(hexstring="0E"), BitVector(hexstring="0B"), BitVector(hexstring="0D")],
        [BitVector(hexstring="0D"), BitVector(hexstring="09"), BitVector(hexstring="0E"), BitVector(hexstring="0B")],
        [BitVector(hexstring="0B"), BitVector(hexstring="0D"), BitVector(hexstring="09"), BitVector(hexstring="0E")]
    ]

    def __init__(self, key : bytearray):
        if len(key) != self.KEY_LENGTH:
            raise Exception("Invalid Key Length!")

        self.round_keys : List[BitVector] = self.key_schedule(key)

    def key_schedule(self, key : bytearray) -> List[BitVector]:
        words : List[BitVector] = []

        rc : BitVector = BitVector(intVal=0x01, size=8)
        x  : BitVector = BitVector(intVal=0x02, size=8)

        for i in range((self.NUM_ROUNDS+1) * self.NUM_WORDS):
            word : BitVector
            if i < self.NUM_WORDS:
                word = BitVector(rawbytes = key[4*i:4*i+4])

            elif i >= self.NUM_WORDS and i % self.NUM_WORDS == 0:
                rcon : BitVector = rc + BitVector(size=24)
                word = words[i-self.NUM_WORDS] ^ self.SubWord(self.RotWord(words[i-1])) ^ rcon
                rc = rc.gf_multiply_modular(x, self.AES_MODULO, 8)

            elif i >= self.NUM_WORDS and self.NUM_WORDS > 6 and i % self.NUM_WORDS == 4:
                word = words[i-self.NUM_WORDS] ^ self.SubWord(words[i-1])

            else:
                word = words[i-self.NUM_WORDS] ^ words[i-1]

            words.append(word)

        round_keys : List[BitVector] = []
        for i in range(0, len(words), self.NUM_WORDS):
            round_key : BitVector = BitVector(size=0)
            for j in range(self.NUM_WORDS):
                round_key += words[i+j]

            round_keys.append(round_key)

        return round_keys

    def SubWord(self, word : BitVector) -> BitVector:
        ret : BitVector = BitVector(size = 0)
        for i in range(0, len(word), 8):
            ret += BitVector(intVal=self.SBOX[word[i : i+8].int_val()], size=8)
        return ret

    def SubWordInv(self, word : BitVector) -> BitVector:
        ret : BitVector = BitVector(size = 0)
        for i in range(0, len(word), 8):
            ret += BitVector(intVal=self.INV_SBOX[word[i : i+8].int_val()], size=8)
        return ret

    def RotWord(self, word : BitVector) -> BitVector:
        return word[8:] + word[:8]

    def ShiftRows(self, block : BitVector) -> BitVector:
        ret : BitVector = BitVector(size=self.BLOCK_SIZE)

        for row in range(0, 4):
            for col in range(0, 4):
                old : int = col * 4 + row
                new : int = ((col + row) % 4) * 4 + row
                ret[old*8 : old*8+8] = block[new*8 : new*8+8]
        return ret

    def ShiftRowsInv(self, block : BitVector) -> BitVector:
        ret : BitVector = BitVector(size=self.BLOCK_SIZE)

        for row in range(0, 4):
            for col in range(0, 4):
                old : int = col * 4 + row
                new : int = ((col - row) % 4) * 4 + row
                ret[old*8 : old*8+8] = block[new*8 : new*8+8]
        return ret

    def MixColumns(self, block : BitVector) -> BitVector:
        ret : BitVector = BitVector(size = self.BLOCK_SIZE)

        for i in range(4):
            for j in range(4):
                for k in range(4):
                    ret[(k*4+i)*8 : (k*4+i)*8+8] ^= self.MIXER[i][j].gf_multiply_modular(block[(k*4+j)*8: (k*4+j)*8 + 8], self.AES_MODULO, 8)
        
        return ret

    def MixColumnsInv(self, block : BitVector) -> BitVector:
        ret : BitVector = BitVector(size = self.BLOCK_SIZE)

        for i in range(4):
            for j in range(4):
                for k in range(4):
                    ret[(k*4+i)*8 : (k*4+i)*8+8] ^= self.INV_MIXER[i][j].gf_multiply_modular(block[(k*4+j)*8: (k*4+j)*8 + 8], self.AES_MODULO, 8)
        
        return ret

    def encrypt(self, plaintext : bytearray) -> bytearray:
        text : bytearray = plaintext[:]

        if len(text) % self.BLOCK_SIZE_IN_BYTES != 0:
            text += self.PADDING * (self.BLOCK_SIZE_IN_BYTES - len(text) % self.BLOCK_SIZE_IN_BYTES)

        for i in range(0, len(text), self.BLOCK_SIZE_IN_BYTES):
            block : BitVector = BitVector(rawbytes=text[i : i+self.BLOCK_SIZE_IN_BYTES])
            text[i : i+self.BLOCK_SIZE_IN_BYTES] = bytearray.fromhex(self.encrypt_block(block).get_bitvector_in_hex())

        return text

    def encrypt_block(self, block : BitVector) -> BitVector:
        block ^= self.round_keys[0]

        for i in range(1, self.NUM_ROUNDS):
            block = self.SubWord(block)
            block = self.ShiftRows(block)
            block = self.MixColumns(block)
            block ^= self.round_keys[i]

        block = self.SubWord(block)
        block = self.ShiftRows(block)
        block ^= self.round_keys[-1]

        return block

    def decrypt(self, ciphertext : bytearray) -> bytearray:
        text : bytearray = ciphertext[:]

        if len(text) % self.BLOCK_SIZE_IN_BYTES != 0:
            raise Exception("Invalid ciphertext length")

        for i in range(0, len(text), self.BLOCK_SIZE_IN_BYTES):
            block : BitVector = BitVector(rawbytes=text[i : i+self.BLOCK_SIZE_IN_BYTES])
            text[i : i+self.BLOCK_SIZE_IN_BYTES] = bytearray.fromhex(self.decrypt_block(block).get_bitvector_in_hex())

        return text

    def decrypt_block(self, block : BitVector) -> BitVector:
        block ^= self.round_keys[-1]
        block = self.ShiftRowsInv(block)
        block = self.SubWordInv(block)
        

        for i in range(1, self.NUM_ROUNDS):
            block ^= self.round_keys[self.NUM_ROUNDS - i]
            block = self.MixColumnsInv(block)
            block = self.ShiftRowsInv(block)
            block = self.SubWordInv(block)

        block ^= self.round_keys[0]

        return block


# Two One Nine Two
# Thats my Kung Fu